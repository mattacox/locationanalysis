...
def hud_risk_band(p):
    if p < 0.20:
        return "Stable / Minimal Risk"
    elif p < 0.35:
        return "At Risk of Displacement"
    elif p < 0.55:
        return "Ongoing Displacement"
    else:
        return "Advanced / Acute Displacement"


long_data_geo["risk_band"] = (
    long_data_geo["displacement_probability"]
    .apply(hud_risk_band)
)
...
long_data_geo["income_up_fast"] = (
    long_data_geo.groupby("GEOID")["median_income"]
    .pct_change()
    .gt(0.15)
)
...
HUD_COLORS = {
    "Stable / Minimal Risk": "#f7fbff",        # very light blue
    "At Risk of Displacement": "#9ecae1",      # light blue
    "Ongoing Displacement": "#fc9272",         # orange
    "Advanced / Acute Displacement": "#cb181d" # deep red
}
...
colormap = cm.StepColormap(
    colors=["#f7fbff", "#9ecae1", "#fc9272", "#cb181d"],
    index=[0.0, 0.20, 0.35, 0.55, 1.0],
    vmin=0,
    vmax=1,
    caption="HUD-Calibrated Displacement Risk"
)
...
for _, row in long_data_geo.iterrows():
    ...
    <strong>Displacement Risk:</strong> {val*100:.1f}%<br>
...
# Build map
m_ts = folium.Map(location=[36.31, -78.59], zoom_start=12, tiles="OpenStreetMap")
...
<div style="
    ...
    font-size: 13px;
">
<b>HUD-Aligned Displacement Typology</b><br><br>

<i style="background:#f7fbff;width:18px;height:18px;float:left;margin-right:8px;border:1px solid #999;"></i>
Stable / Not Vulnerable<br>

<i style="background:#9ecae1;width:18px;height:18px;float:left;margin-right:8px;border:1px solid #999;"></i>
At Risk of Displacement<br>

<i style="background:#fc9272;width:18px;height:18px;float:left;margin-right:8px;border:1px solid #999;"></i>
Ongoing Displacement<br>

<i style="background:#cb181d;width:18px;height:18px;float:left;margin-right:8px;border:1px solid #999;"></i>
Advanced / Acute Displacement
</div>
{% endmacro %}
"""
...
m_ts.get_root().add_child(legend)
...
m_ts.save("html/displacement_risk_timeslider_bgv1.html")
print("✅ BG-level TimeSlider saved.")
