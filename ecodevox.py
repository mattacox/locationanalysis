...
# Define your Oxford tracts
    ...
    # Inside the for-loop, after building GEOID:
    data["GEOID"] = data["STATE"] + data["COUNTY"] + data["TRACT"] + data["BLOCK_GROUP"]
    # Filter to Oxford block groups only (first 11 digits = tract)
    data = data[data["TRACT"].isin(oxford_tracts)].copy()
    print(f"{vintage} Oxford rows: {len(data)}")
...
if long_data_geo.empty:
    raise ValueError("No data available for Oxford tracts across the selected years.")
elif long_data_geo[risk_fields].dropna().empty:
    raise ValueError("All risk fields are NaN or missing after filtering. Check input data.")
...
for indicator in indicators:
    ...
    for _, row in long_data_geo.iterrows():
        ...
        popup_html = (
            ...
            f"<strong>GEOID:</strong> {row['GEOID']}<br>"
            f"<strong>{indicator.replace('_', ' ').title()}:</strong> {popup_val}"
        )
    ...
    # Save to file
    output_path = f"html/ox_{indicator}_timeslider.html"
    m.save(output_path)
