...
def run_brand_analysis(
    ...
    df["GEOID_T"] = df["GEOID_T"].astype(str)
    ...
    tract_jobs["GEOID_POI"] = tract_jobs["GEOID_jobs"].astype(str)
    ...
    # Merge to bring in COUNTY from merged_df based on GEOID
    stores_gdf = stores_gdf.merge(
    df[['GEOID', 'COUNTY']],
    how='left',
    left_on='GEOID_POI',
    right_on='GEOID'
    ...
    def safe_fillna(gdf):
        """Fill NaNs only in non-geometry columns."""
        non_geom_cols = gdf.select_dtypes(exclude="geometry").columns
        gdf[non_geom_cols] = gdf[non_geom_cols].fillna(0)
        return gdf

    # Merge pipeline
    merged_df = pd.merge(df, store_presence, left_on="GEOID", right_on="GEOID_POI", how="left")
    merged_df = pd.merge(merged_df, road_df, left_on="GEOID", right_on="GEOID_road", how="left")
    merged_df = pd.merge(merged_df, ruca_df, left_on="GEOID_T", right_on="GEOID_ruca", how="left")
    merged_df = pd.merge(merged_df, tract_jobs, left_on="GEOID", right_on="GEOID_jobs", how="left")
    merged_df = pd.merge(merged_df, landuse_gdf, left_on="GEOID_T", right_on="GEOID_landuse", how="left")
    print(merged_df.tail())

    # One-time safe fillna
    merged_df = safe_fillna(merged_df)
...
# Convert X back to DataFrame with original column names
    ...
    tracts = gpd.GeoDataFrame(df, geometry="geometry", crs="EPSG:4326")
    # Optional map
    if make_map:
        tracts["predicted_prob"] = tracts["predicted_prob"]
        tracts["predicted_prob_pct"] = tracts["predicted_prob_pct"]
        tracts["median_income"] = tracts["median_income"]
        tracts["pop_density"] = tracts["pop_density"]
        tracts["population"] = tracts["population"]
        tracts["area_km2"] = tracts["area_km2"]
        tracts["road_access_score"] = tracts["road_access_score"]
        # tracts["GEOIDFQ_split"] = tracts["GEOID"].astype(str)
        tracts["pct_with_degree"] = (tracts["pct_with_degree"]* 100).round(1).astype(str) + "%"
        tracts["pct_owner_occupied"] = (tracts["pct_owner_occupied"]* 100).round(1).astype(str) + "%"
        tracts["total_jobs"] = tracts["total_jobs"]
        tracts["job_density"] = tracts["job_density"]
        tracts["entropy"] = tracts["entropy"]

        tracts["share_high_wage"] = tracts["share_high_wage"]
        tracts["share_mid_wage"] = tracts["share_mid_wage"]
        tracts["share_low_wage"] = tracts["share_low_wage"]
        tracts["pct_commercial"] = tracts["pct_commercial"]
        ...
        tracts_clean = df.dropna(subset=["predicted_prob"])
        predicted_tracts_focus = tracts_clean[tracts_clean["COUNTY"].isin(focus_counties)]
        ...
        stores_focus = stores_focus[stores_focus["COUNTY"].isin(focus_counties)]
        ...
        m.save(f"html/{brand.replace(' ', '_')}_block_map.html")

        print(f"Map saved to html/brands/{brand.replace(' ', '_')}_block_map.html")
...
def main():
    ...
    model, coef_df, top10_df, coef_df1 = run_brand_analysis(
        brand=brand,
        store_data_path="data/poisblock.csv",
